...
def download(item):
	...
	if any(s in item for s in dataTypesToDownload):
		...
		try:
			if "?" in item:
				download1 = open(base_path + "/" + item.split("?")[len(item.split("?")) - 2], "wb")
			else:
				download1 = open(base_path + "/" + item, "wb")
			print("Downloading {} to {}".format(item, download1.name))
			if external:
...
if len(sys.argv) == 1:
    ...
    url = "https://login.bankhapoalim.co.il/ng-portals/auth/he/?reqName=getLogonPage"
else:
	if sys.argv[1] == "-h":
